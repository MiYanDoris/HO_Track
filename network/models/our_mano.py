'''
Our modified version of Manolayer.
Fix the small problem in the official implementation of MANO hand: the hand root is not in origin
'''
...
class OurManoLayer(Module):
    ...
    def __init__(self,
                side='right',
                mano_root='/home/jiayichen/manopth/mano/models',
                ):
        super().__init__()
        self.rot = 3
        self.side = side
        self.ncomps = 45
        if not os.path.isdir(mano_root):
            mano_root = '/home/hewang/Desktop/jiayi/manopth/mano/models'
        if side == 'right':
