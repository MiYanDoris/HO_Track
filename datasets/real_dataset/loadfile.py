...
def outlier_removal(hand_cld, gt_vertices):
    ...
    fg_mask = np.where(distance < 0.03)[0] 
    hand_cld = hand_cld[fg_mask]
    return hand_cld

def path_list2plainobj_input_check(cam_in_path, dpt_path, mask_path, anno_path, hand_vertices=None, denoise=True, color_pcld_save_path=None):
    rot, trans, dim = read_rtd(anno_path)
    ...
    depth2d = np.array(depth2d, dtype=np.float32) / 1000
    obj_mask, hand_mask = read_mask(clean_mask, dscale=1, crop=crop_list)
    large_mask, large_hand_mask = read_mask(clean_mask, dscale=1)
    depth3d_hand = o3d.geometry.Image(depth2d * (1 - large_hand_mask[..., 0]))
    depth3d_obj = o3d.geometry.Image(depth2d * large_mask[..., 0])
    depth2d = depth2d[x1:x2, y1:y2]
    hand_mask[np.where(depth2d < 0.001)] = 0
    ...
    rot_matrix = Rt.from_rotvec(rot).as_matrix()
    obj_pcd = obj_pcd @ rot_matrix - rot_matrix.T @ trans
    obj_pcd = crop3d(dim, 2, obj_pcd)
    obj_pcd = obj_pcd @ rot_matrix.T + trans
    ...
    if hand_vertices is not None:
        hand_pcd = outlier_removal(hand_pcd, hand_vertices)

    if color_pcld_save_path is not None:
        color_img = o3d.io.read_image(dpt_path.replace('depth', 'image').replace('png', 'jpg'))
        rgbd_image = o3d.geometry.RGBDImage.create_from_color_and_depth(color_img,
                                                                        depth3d_obj,
                                                                        depth_scale=1,
                                                                        convert_rgb_to_intensity=False)
        obj_color_pcd = o3d.geometry.PointCloud.create_from_rgbd_image(
            rgbd_image, intrinsics)
        o3d.io.write_point_cloud(color_pcld_save_path, obj_color_pcd)

    return rot_matrix, trans, obj_mask, hand_mask, depth2d, obj_pcd, hand_pcd, camMat, crop_list, dim
